...
const FixedHolder = forwardRef<HTMLDivElement, FixedHeaderProps<unknown>>(
  (
    ...
    const flattenColumnsWithScrollbar: readonly ColumnType<unknown>[] = useMemo(
      () =>
    ...
    const headerStickyOffsets: {
      ...
      return {
        ...
        left:
          direction === 'rtl'
            ? [...left.map((width) => width + combinationScrollBarSize), 0]
            : left,
        right:
          direction === 'rtl'
            ? right
    ...
    const mergedColumnWidth: number[] = useColumnWidth(colWidths, columCount);
